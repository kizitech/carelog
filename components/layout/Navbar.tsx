  ...
  return (
    <>
      <header className="sticky top-0 z-50 bg-card/90 backdrop-blur-xl border-b border-border shadow-sm">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-[58px] flex items-center gap-4">
          ...
          <Link href="/dashboard" className="flex items-center gap-2.5 flex-shrink-0">
            ...
            <div className="hidden sm:block">
              <p className="text-[14px] font-extrabold text-blue-700 dark:text-blue-400 tracking-tight leading-none">CareLog</p>
            ...
            <div className="relative">
              <button onClick={() => { setUserMenu(o => !o); setThemeMenu(false); }}
                ...
                <div className="hidden md:flex flex-col items-start">
                  <span className="text-[12px] font-semibold text-foreground leading-tight">{user.name.split(" ")[0]}</span>
                  <span className="text-[10px] text-muted-foreground leading-tight">{user.role}</span>
                </div>
                <ChevronDown className={cn("w-3.5 h-3.5 text-muted-foreground transition-transform", userMenu && "rotate-180")} />
              </button>
                ...
                {userMenu && (
                  <motion.div initial={{ opacity: 0, scale: 0.95, y: -4 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: -4 }} transition={{ duration: 0.13 }}
                    ...
                    <div className="px-3.5 py-2.5 border-b border-border">
                      ...
                      <p className="text-xs text-muted-foreground truncate">{user.wing}</p>
                    </div>
                    <div className="py-1">
                      <Btn icon={<User className="w-3.5 h-3.5" />} onClick={() => { setProfileOpen(true); setUserMenu(false); }}>My Profile</Btn>
                      <Btn icon={<Settings className="w-3.5 h-3.5" />} onClick={() => { setProfileOpen(true); setUserMenu(false); }}>Settings</Btn>
                    </div>
                    <div className="border-t border-border py-1">
                      <Btn icon={<RotateCcw className="w-3.5 h-3.5" />} onClick={() => { setResetOpen(true); setUserMenu(false); }}>Reset Demo Data</Btn>
                      <Btn icon={<LogOut className="w-3.5 h-3.5" />} onClick={() => { logout(); router.push("/login"); }} danger>Sign Out</Btn>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
      ...
      <ConfirmDialog open={resetOpen} onClose={() => setResetOpen(false)} onConfirm={() => { resetApp(); toast.success("Demo data restored"); }}
        title="Reset Demo Data" confirmLabel="Yes, Reset" danger
        description="This will clear all current shift logs and restore the original demo data. This cannot be undone." />
    </>
  );
}

function Btn({ icon, children, onClick, danger }: { icon: React.ReactNode; children: string; onClick: () => void; danger?: boolean }) {
  return (
