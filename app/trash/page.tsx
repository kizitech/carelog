...
export default function TrashPage() {
  const { trashedRecords, restoreRecord, deleteRecord, clearTrash } = useApp();
  const [clearOpen, setClearOpen] = useState(false);

  const handleRestore = (id: string, name: string) => {
    restoreRecord(id);
    toast.success("Record restored", { description: name });
  };

  const handleDelete = (id: string, name: string) => {
    deleteRecord(id);
    toast.error("Record permanently deleted", { description: name });
  };
  ...
  return (
    <AppShell>
        ...
        >
          <div>
            ...
            <p className="text-muted-foreground mt-1 text-[15px]">
              {trashedRecords.length} trashed record{trashedRecords.length !== 1 ? "s" : ""}
            </p>
          </div>
        ...
        {trashedRecords.length > 0 && (
          ...
          >
            ...
            <p className="text-sm text-amber-800 dark:text-amber-300">
              Trashed records are hidden from the dashboard and resident histories.
              Restore them to make them visible again, or delete them permanently.
            </p>
          </motion.div>
        )}
        ...
        {trashedRecords.length === 0 ? (
          <div className="bg-card border border-border rounded-2xl shadow-sm">
            <EmptyState
              ...
              description="Shift records you move to trash will appear here. You can restore or permanently delete them."
            />
          </div>
        ) : (
          <div className="space-y-3">
            <AnimatePresence>
              {trashedRecords.map((record, idx) => (
                <motion.div
                  key={record.id}
                  initial={{ opacity: 0, y: 6 }}
                  animate={{ opacity: 1, y: 0 }}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="text-sm font-bold text-foreground">{record.residentName}</p>
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">
                      By {record.staffName} · {formatDateShort(record.timestamp)}
                    </p>
                    {record.observations && (
                      <p className="text-xs text-muted-foreground truncate max-w-sm mt-0.5 opacity-70">
                        {record.observations}
                      </p>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <button
                      onClick={() => handleRestore(record.id, record.residentName)}
                    ...
                    <button
                      onClick={() => handleDelete(record.id, record.residentName)}
      ...
      <ConfirmDialog
        ...
        onConfirm={() => { clearTrash(); toast.error("All trashed records permanently deleted"); }}
        title="Clear All Trash"
        confirmLabel="Clear All"
        danger
        description={`This will permanently delete all ${trashedRecords.length} trashed record${trashedRecords.length !== 1 ? "s" : ""}. This cannot be undone.`}
      />
    </AppShell>
  );
}
